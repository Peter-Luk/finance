def encoder(*args, **kwargs):
    ...
    try:
        if args:
            mfp = args[0]
        if 'mfp' in list(kwargs.keys()):
            mfp = kwargs['mfp']
        if 'case' in list(kwargs.keys()):
            case = kwargs['case']
...
#             else: la = kwargs['alias']
            ...
            elif isinstance(kwargs['alias'], tuple):
                la = list(kwargs['alias'])
