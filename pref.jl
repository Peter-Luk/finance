...
setproperty!(frame, "columns", [toupper(n, 1, 1) for n in frame.columns.tolist()])
end
if lowercase(string_format) == "upper"
setproperty!(frame.index, "name", uppercase(frame.index.name))
setproperty!(frame, "columns", [uppercase(n) for n in frame.columns.tolist()])
end
if lowercase(string_format) == "lower"
setproperty!(frame.index, "name", lowercase(frame.index.name))
setproperty!(frame, "columns", [lowercase(n) for n in frame.columns.tolist()])
end
return frame
end
py"""
