...
db_name, db_table = pref.db['Equities']['name'], pref.db['Equities']['table']
...
3.) SQLite db, type connection.
    ...
    qstr = "UPDATE {} SET {{}} WHERE eid={:d} AND date='{{:%Y-%m-%d}}'".format(
            db_table, code)
    for _ in list(s_dict.keys()):
        sl = []
        try:
            for __ in pref.fields:
                if __ == 'volume':
                    sl.append('{}={:d}'.format(
                        __, int(s_dict[_][__.capitalize()])))
                else:
                    sl.append('{}={:.3f}'.format(
                        __, s_dict[_][__.capitalize()]))
...
def pstored(*args, **kwargs):
    ...
    start = get_start(1)
    qstr = "SELECT {} FROM {} WHERE eid={{:d}} AND date BETWEEN \
    '{{:%Y-%m-%d}}' AND '{{:%Y-%m-%d}}' ORDER BY date ASC".format(', '.join(
        pref.fields + ['date']), db_table)
    d = pd.read_sql_query(qstr.format(a0, start, end), scon, parse_dates={
        'date': '%Y-%m-%d'}).transpose().to_dict().values()
    if d:
        vd = {}
        for _ in d:
            vd[_.pop('date')] = _
        for _ in list(vd.keys()):
            temp = {}
            for __ in list(vd[_].keys()):
                temp[__.capitalize()] = vd[_][__]
            vd[_] = temp
        return vd


def stored_data(*args, **kwargs):
    res, where, fields = [], [], ['date'] + pref.fields
    lk = list(kwargs.keys())
    try:
        a0 = args[0]
    ...
    if 'where' in lk:
        if isinstance(kwargs['where'], list):
            where.extend(kwargs['where'])
        elif isinstance(kwargs['where'], str):
    ...
    if where:
        ...
        fields.append('id')
        res.extend(conn.cursor().execute(qstr.format(
...
update 'local' database.
    ...
    if 'equities_id' in lkw:
        if isinstance(kwargs['equities_id'], int):
            aid = [kwargs['equities_id']]
        ...
        elif isinstance(kwargs['equities_id'], list):
            aid = kwargs['equities_id']
    wdp = web_collect(aid, period=period)
    for _ in aid:
        idate = []
        atd = [i['date'] for i in conn.cursor().execute(
            "SELECT {} FROM {} WHERE eid={:d}".format(
                ', '.join(pref.fields + ['date']), db_table, _)).fetchall()]
        ...
        if idate:
            for i in idate:
                ...
                # if wi['Volume']:
                    ...
                    if wi['Volume'] > 0:
                        imk = list(im.keys())
                        conn.cursor().execute(istr.format(
                            db_table,
                            ','.join(imk),
                            ','.join([
                                '{{{}}}'.format(j) for j in imk])).format(
                                    **im))
                        conn.commit()
                        ic += 1
                except:
...
1). Download .csv format data file with corresponding varied stock code for
finance.yahoo.com search to 'Download' folder, file will automatically
'wipe' off from 'Download' folder, and
    ...
    if cp:
        ...
        for _ in listdir(cp):
            if path.isfile(sep.join((cp, _))):
                tfl = _.split('.')
                if tfl[-1] == folder and len(tfl[:-1]) == 2:
                    af.append(_)
        for _ in af:
        ...
        try:
            ...
            if 'path' in lkkeys:
                ...
                if isinstance(kpath, str):
                    self.__csv_fullpath = kpath
                if isinstance(kpath, dict):
                    if 'db' in list(kpath.keys()):
...
first positional or 'code' named is name in file system.
        ...
        while i < len(values):
            ...
            tf = [hdr[_] for _ in ['open', 'high', 'low', 'close']]
            c, _ = [], 0
            while _ < len(tf) - 1:
                r = False
                if tf[_] == tf[_+1]:
